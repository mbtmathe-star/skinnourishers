...
import {
  ...
  Heart, Lightbulb, Mail, MapPin, Phone, Quote, Sparkles, Star,
  Target, UserRound, X
} from 'lucide-react';
...
function Kicker({ children, className = '' }) {
  return (
    <span className={`inline-flex items-center gap-3 text-[11px] font-body uppercase tracking-[0.24em] text-primary ${className}`}>
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 overflow-y-auto" onClick={close}>
    <motion.div initial={{ opacity: 0, scale: .95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-2xl my-8 bg-white rounded-3xl shadow-xl shadow-primary/5 border border-primary/10 p-8 md:p-10" onClick={(e) => e.stopPropagation()}>
      <div className="flex items-center justify-between mb-6">
        <h3 className="font-heading text-2xl text-foreground flex items-center gap-2"><Sparkles className="w-5 h-5 text-primary" /> Online Skin Assessment</h3>
...
function FounderSection() {
  const features = [{ icon: Target, title: 'Passion for Results', description: "Every client's transformation is personal to us" }, { icon: Heart, title: 'Client-Centered Care', description: 'Your unique skin journey guides everything we do' }, { icon: Lightbulb, title: 'Continuous Innovation', description: 'Always learning and bringing you the latest in skincare' }];
  return <section className="py-16 lg:py-24 relative overflow-hidden"><div className="absolute inset-0 bg-gradient-to-br from-rose-light/50 via-secondary/60 to-white" /><div className="absolute inset-0 bg-gradient-to-tl from-primary/5 via-transparent to-transparent" /><div className="container relative z-10"><div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center"><motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="relative"><div className="aspect-[4/5] overflow-hidden rounded-2xl shadow-2xl"><img src="/assets/about-image-BmoL2o4f.png" alt="Sonia - Founder of Skin Nourishers" className="w-full h-full object-cover object-top" /><div className="absolute inset-0 bg-gradient-to-t from-primary/20 via-transparent to-transparent" /></div><div className="absolute -bottom-4 -left-4 lg:-left-8 bg-gradient-to-br from-primary to-rose px-6 py-4 rounded-2xl shadow-xl shadow-primary/30"><div className="text-center"><div className="font-heading text-3xl text-primary-foreground font-light">15+</div><div className="text-xs text-primary-foreground/80 uppercase tracking-elegant font-body">Years</div></div></div></motion.div><motion.div initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}><Kicker className="mb-4">The person behind the clinic</Kicker><h2 className="font-heading text-4xl md:text-5xl font-light text-foreground mb-6 leading-tight">A Journey of<br /><span className="italic text-primary">Passion & Purpose</span></h2><div className="w-16 h-px bg-primary/50 mb-8" /><p className="text-muted-foreground font-body leading-relaxed mb-6">Skin Nourishers was born from a deeply personal journey. After struggling with my own skin concerns for years and experiencing the frustration of ineffective treatments, I made it my mission to create a space where real results meet genuine care.</p><p className="text-muted-foreground font-body leading-relaxed mb-6">What began as a passion for helping others achieve healthy, radiant skin has grown into a trusted destination for clients across Sandton and beyond. Every treatment we offer reflects the same dedication I would give to my own skin—because I understand what it means to want real change.</p><p className="text-muted-foreground font-body leading-relaxed mb-10">At Skin Nourishers, we don't just treat skin—we build confidence. Our philosophy is simple: listen deeply, treat thoughtfully, and celebrate every transformation together with our clients.</p><div className="grid gap-5 mb-10">{features.map((item) => <div key={item.title} className="flex items-start gap-4"><item.icon className="w-6 h-6 text-primary flex-shrink-0 mt-0.5" /><div><h3 className="font-heading text-lg text-foreground mb-1">{item.title}</h3><p className="text-muted-foreground text-sm font-body">{item.description}</p></div></div>)}</div><Link to="/about" className="inline-flex items-center gap-3 text-primary hover:text-primary/80 transition-colors duration-300 font-body text-sm uppercase tracking-wide-elegant group"><span>Read Our Full Story</span><ArrowRight className="w-4 h-4 transition-transform duration-300 group-hover:translate-x-2" /></Link></motion.div></div></div></section>;
}
